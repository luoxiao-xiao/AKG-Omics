...
LAMBDA_CYCLE = _env_float("LAMBDA_CYCLE", 0.2)
LAMBDA_PARTIAL_CYCLE = _env_float("LAMBDA_PARTIAL_CYCLE", LAMBDA_CYCLE)
LAMBDA_BRIDGE_ALIGN = _env_float("LAMBDA_BRIDGE_ALIGN", 0.5)
LAMBDA_ALIGN = _env_float("LAMBDA_ALIGN", 0.02)
MET_CLEAN_LOSS = _env_bool("MET_CLEAN_LOSS", False)
...
MET_PRECISION_PROFILE = _env_str("MET_PRECISION_PROFILE", "off")
MET_PRECISION_PROFILES = {"1", "true", "yes", "agent_precision_v1", "precision_v1"}
...
MET_KB_AGENT_REUSE_SUCCESSFUL_SELECTION = _env_bool("MET_KB_AGENT_REUSE_SUCCESSFUL_SELECTION", True)
MET_KB_AGENT_STRICT_FAILURE = _env_bool("MET_KB_AGENT_STRICT_FAILURE", False)
...
MET_DEFAULT_HMDB_PPM_TOL = _env_float("MET_DEFAULT_HMDB_PPM_TOL", PPM_TOL)
MET_DEFAULT_HMDB_TOPK_CANDIDATES = _env_int("MET_DEFAULT_HMDB_TOPK_CANDIDATES", TOPK_CANDIDATES)
MET_DEFAULT_HMDB_MIN_CANDIDATE_WEIGHT = _env_float("MET_DEFAULT_HMDB_MIN_CANDIDATE_WEIGHT", MIN_CANDIDATE_WEIGHT)
...
PROTEO_MODE_TAG = (
    f"g{PROTEO_GRID_SIZE}_he{PROTEO_HE_DIM}_gene{PROTEO_GENE_TOPK}_glat{PROTEO_GENE_LATENT_DIM}"
...
def met_precision_enabled():
    return str(MET_PRECISION_PROFILE).strip().lower() in MET_PRECISION_PROFILES


def met_clean_loss_value(name, default):
    if not MET_CLEAN_LOSS:
        return default
    clean_defaults = {
        "lambda_pcc": 0.0,
        "lambda_latent": 0.0,
        ...
        "lambda_dgi": 0.0,
        "lambda_graph": 0.0,
        "lambda_bridge_align": 0.0,
        "target_soft_alpha": 0.0,
        "disable_soft_gate": True,
    }
    return clean_defaults.get(str(name), default)


def _met_task_env_prefix(task_id):
    task_id = str(task_id or "").strip().lower()
    if task_id.startswith("task5"):
        return "MET_TASK5"
    if task_id.startswith("task6"):
        return "MET_TASK6"
    if task_id.startswith("task7"):
        return "MET_TASK7"
    if task_id.startswith("task8"):
        return "MET_TASK8"
    if task_id.startswith("warmup"):
        return "MET_WARMUP"
    return "MET_TASK"


def _met_precision_sources_from_env(prefix, default_sources):
    raw = os.getenv(f"{prefix}_SOURCES", "")
    if raw.strip():
        return [x.strip().lower() for x in raw.replace(",", ";").split(";") if x.strip()]
    return list(default_sources)


def get_met_task_precision_config(task_id):
    if not met_precision_enabled():
        return {}
    task_id = str(task_id or "").strip().lower()
    prefix = _met_task_env_prefix(task_id)
    defaults = {
        ...
        "task7_he_to_metabolism": {
            "sources": ["reactome", "hmdb", "chebi"],
            "lambda_kb": 0.05,
            "lambda_graph": 0.075,
            "lambda_pcc": 0.03,
            "disable_soft_gate": False,
            "hmdb_ppm_tol": 5.0,
            "hmdb_topk_candidates": 2,
            "hmdb_min_candidate_weight": 0.05,
        },
        ...
        "warmup_he_metabolism_to_gene": {
            "sources": ["reactome"],
            "hmdb_ppm_tol": MET_DEFAULT_HMDB_PPM_TOL,
            "hmdb_topk_candidates": MET_DEFAULT_HMDB_TOPK_CANDIDATES,
            "hmdb_min_candidate_weight": MET_DEFAULT_HMDB_MIN_CANDIDATE_WEIGHT,
        },
    }
    cfg = dict(defaults.get(task_id, {"sources": ["reactome"]}))
    cfg["sources"] = _met_precision_sources_from_env(prefix, cfg.get("sources", ["reactome"]))
    cfg["lambda_kb"] = _env_float(f"{prefix}_LAMBDA_KB", float(cfg.get("lambda_kb", LAMBDA_KB)))
    cfg["lambda_graph"] = _env_float(f"{prefix}_LAMBDA_GRAPH", float(cfg.get("lambda_graph", LAMBDA_GRAPH)))
    cfg["lambda_pcc"] = _env_float(f"{prefix}_LAMBDA_PCC", float(cfg.get("lambda_pcc", LAMBDA_PCC)))
    cfg["lambda_cycle"] = _env_float(f"{prefix}_LAMBDA_CYCLE", float(cfg.get("lambda_cycle", LAMBDA_CYCLE)))
    cfg["lambda_partial_cycle"] = _env_float(f"{prefix}_LAMBDA_PARTIAL_CYCLE", float(cfg.get("lambda_partial_cycle", cfg["lambda_cycle"])))
    cfg["lambda_align"] = _env_float(f"{prefix}_LAMBDA_ALIGN", float(cfg.get("lambda_align", LAMBDA_ALIGN)))
    cfg["disable_soft_gate"] = _env_bool(f"{prefix}_DISABLE_SOFT_GATE", bool(cfg.get("disable_soft_gate", DISABLE_SOFT_GATE)))
    cfg["hmdb_ppm_tol"] = _env_float(f"{prefix}_HMDB_PPM_TOL", float(cfg.get("hmdb_ppm_tol", MET_DEFAULT_HMDB_PPM_TOL)))
    cfg["hmdb_topk_candidates"] = _env_int(f"{prefix}_HMDB_TOPK_CANDIDATES", int(cfg.get("hmdb_topk_candidates", MET_DEFAULT_HMDB_TOPK_CANDIDATES)))
    cfg["hmdb_min_candidate_weight"] = _env_float(
        f"{prefix}_HMDB_MIN_CANDIDATE_WEIGHT",
        float(cfg.get("hmdb_min_candidate_weight", MET_DEFAULT_HMDB_MIN_CANDIDATE_WEIGHT)),
    )
    cfg["precision_profile"] = str(MET_PRECISION_PROFILE)
    return cfg


def apply_met_task_precision_trainer_overrides(trainer_kwargs, task_id):
    cfg = get_met_task_precision_config(task_id)
    if not cfg:
        return trainer_kwargs
    out = trainer_kwargs
    out["lambda_pcc"] = float(cfg.get("lambda_pcc", out.get("lambda_pcc", LAMBDA_PCC)))
    out["lambda_cycle"] = float(cfg.get("lambda_cycle", out.get("lambda_cycle", LAMBDA_CYCLE)))
    out["lambda_partial_cycle"] = float(cfg.get("lambda_partial_cycle", out.get("lambda_partial_cycle", out["lambda_cycle"])))
    out["lambda_align"] = float(cfg.get("lambda_align", out.get("lambda_align", LAMBDA_ALIGN)))
    out["disable_soft_gate"] = bool(cfg.get("disable_soft_gate", out.get("disable_soft_gate", DISABLE_SOFT_GATE)))
    return out

def _met_available_sources(local_status, preferred=None):
    preferred = list(preferred or MET_KB_ALLOWED_SOURCES)
    out = []
    for sid in preferred:
        sid = str(sid).strip().lower()
        if sid and sid in {"hmdb", "reactome", "chebi"} and local_status.get(sid, False) and sid not in out:
            out.append(sid)
    return out


def _met_agent_metric_safe_sources(task_id, source_modalities, target_modality, local_status):
    """Constrain the LLM source choice to KB sources that were stable for each met task."""
    task_id = str(task_id or "").strip().lower()
    src = {str(x).strip().lower() for x in (source_modalities or [])}
    target = str(target_modality or "").strip().lower()
    precision_cfg = get_met_task_precision_config(task_id)
    if precision_cfg.get("sources"):
        selected = _met_available_sources(local_status, precision_cfg.get("sources"))
        if selected:
            return selected

    # Current metabolite feature names are m/z-like values. HMDB/ChEBI exact-mass
    # evidence can be noisy for gene-target tasks, so Reactome is the safe bridge.
    policy = {
        "task5_he_gene_to_metabolism": ["reactome"],
        "task6_he_metabolism_to_gene": ["reactome"],
        "task7_he_to_metabolism": ["reactome", "hmdb", "chebi"],
        "task8_he_to_gene_in_metabolomics": ["reactome"],
        "warmup_he_metabolism_to_gene": ["reactome"],
    }
    preferred = policy.get(task_id)
    if preferred is None:
        if target == "gene":
            preferred = ["reactome"]
        elif target == "metabolism" and "gene" not in src:
            preferred = ["reactome", "hmdb", "chebi"]
        else:
            preferred = ["reactome"]
    selected = _met_available_sources(local_status, preferred)
    if selected:
        return selected
    fallback = _met_available_sources(local_status, ["reactome", "hmdb", "chebi"])
    return fallback


def _apply_met_agent_metric_safe_selection(selection_dict, raw_selected, task_id, source_modalities, target_modality, local_status):
    raw_selected = [
        str(s).strip().lower()
        for s in (raw_selected or [])
        if str(s).strip().lower() in {"hmdb", "reactome", "chebi"}
    ]
    raw_selected = list(dict.fromkeys(raw_selected))
    safe_selected = _met_agent_metric_safe_sources(
        task_id=task_id,
        source_modalities=source_modalities,
        target_modality=target_modality,
        local_status=local_status,
    )
    selection_out = dict(selection_dict or {})
    selection_out["agent_raw_selected_source_ids"] = raw_selected
    selection_out["agent_metric_safe_policy_applied"] = True
    selection_out["agent_metric_safe_policy_reason"] = (
        "Metabolomics features are m/z-like; HMDB/ChEBI mass-level links are kept only "
        "for HE-only metabolism prediction, while Reactome is used as the stable bridge "
        "for gene-target or gene-conditioned tasks."
    )
    selection_out["selected_source_ids"] = list(safe_selected)
    notes = list(selection_out.get("notes") or [])
    notes.append("met_agent_metric_safe_source_guard_applied")
    if raw_selected != safe_selected:
        notes.append("agent_raw_selection_overridden_for_metric_stability")
    selection_out["notes"] = notes
    return list(safe_selected), selection_out

def _load_previous_successful_met_agent_selection(task_id, orchestration_dir, current_report_path=None):
    """Reuse a previous successful LLM source selection for the same task."""
    if not orchestration_dir:
        return None, None
    try:
        import glob as _glob
        import json as _json
        pattern = os.path.join(str(orchestration_dir), f"kb_orchestration_{task_id}_rep*_seed*.json")
        current_abs = os.path.abspath(current_report_path) if current_report_path else ""
        candidates = []
        for path in _glob.glob(pattern):
            ...
            try:
                ...
                continue
            selection = report.get("selection", {}) if isinstance(report, dict) else {}
            if not isinstance(selection, dict):
                continue
            mode = str(selection.get("mode", "")).strip().lower()
            if mode not in {"agent_only", "agent+rule"}:
                continue
            final = report.get("final_selected_sources") or selection.get("selected_source_ids") or []
            final = [
                str(x).strip().lower()
                for x in final
                if str(x).strip().lower() in {"hmdb", "reactome", "chebi"}
            ]
            final = list(dict.fromkeys(final))
            if not final:
                continue
            candidates.append((os.path.getmtime(path), path, report, final))
        if not candidates:
            return None, None
        candidates.sort(reverse=True, key=lambda x: x[0])
        _, path, report, final = candidates[0]
        cached = dict(report)
        cached["met_agent_reused_from_report"] = path
        cached["final_selected_sources"] = final
        return final, cached
    except Exception as exc:
        return None, {"met_agent_reuse_error": str(exc)}
...
def select_sources_for_met_task_with_agent(task_id, source_modalities, target_modality, local_status, report_path):
    ...
    raw_selected = list(dict.fromkeys(raw_selected))
    agent_required_but_failed = REQUIRE_MET_KB_LLM_AGENT and selection.mode not in {"agent_only", "agent+rule"}
    if agent_required_but_failed:
        cached_selected, cached_report = (None, None)
        if MET_KB_AGENT_REUSE_SUCCESSFUL_SELECTION:
            cached_selected, cached_report = _load_previous_successful_met_agent_selection(
                task_id=task_id,
                orchestration_dir=os.path.dirname(report_path),
                current_report_path=report_path,
            )
        if cached_selected:
            report = dict(cached_report or {})
            report["task"] = task_spec.to_dict()
            report["domain"] = "metabolomics"
            report["selection_mode_requested"] = MET_KB_SELECTION_MODE
            report["requested_mode"] = MET_KB_REQUESTED_MODE
            report["use_met_kb_agent"] = bool(USE_MET_KB_AGENT)
            report["use_data_profile"] = bool(MET_KB_USE_DATA_PROFILE)
            report["selection_policy"] = MET_KB_SELECTION_POLICY
            report["local_source_status"] = {s: bool(local_status.get(s, False)) for s in allowed}
            report["allowed_sources"] = allowed
            report["agent_required_but_current_llm_failed"] = True
            report["agent_current_failed_selection"] = selection.to_dict()
            report["agent_selection_reuse_enabled"] = True
            report["final_selected_sources"] = cached_selected
            print(
                f">>> [KB-SELECT][WARN] {task_id} LLM selection failed; reused previous successful agent selection={';'.join(cached_selected)}",
                flush=True,
            )
            return cached_selected, _write_met_selection_report(report_path, report)
        if MET_KB_AGENT_STRICT_FAILURE:
            raise RuntimeError(
        ...
        print(
            f">>> [KB-SELECT][WARN] {task_id} LLM selection failed; continuing with deterministic fallback selection",
            flush=True,
        )
    if not raw_selected:
        raw_selected = [s for s in allowed if local_status.get(s, False)]
    selected, selection_report = _apply_met_agent_metric_safe_selection(
    ...
    report = {
        ...
        "agent_raw_selected_sources": raw_selected,
...
def select_sources_for_met_task(task_id, source_modalities, target_modality, repeat_idx=None, seed=None, orchestration_dir=None):
    ...
    precision_cfg = get_met_task_precision_config(task_id)
    if precision_cfg.get("sources"):
        precision_selected = _met_available_sources(local_status, precision_cfg.get("sources"))
        if precision_selected:
            selected = precision_selected
            report["met_precision_source_override"] = True
            report["met_precision_config"] = precision_cfg
    report["domain"] = "metabolomics"
...
def run_detailed_task_analysis(task_name, repeat_idx, seed, history, gt, pred, graph_eval, adata_for_names_and_coords, target_task):
    ...
    if coords is not None:
        coords = np.asarray(coords, dtype=np.float32)
        np.save(os.path.join(task_out_dir, "coords.npy"), coords)
...
def build_met_task_cache_path(
    ...
    cache_meta = {
        ...
        "met_precision_profile": str(MET_PRECISION_PROFILE),
        "met_clean_loss": bool(MET_CLEAN_LOSS),
        "lambda_target": float(LAMBDA_TARGET),
        "lambda_cycle": float(LAMBDA_CYCLE),
        "lambda_partial_cycle": float(LAMBDA_PARTIAL_CYCLE),
        "lambda_kb": float(LAMBDA_KB),
        "lambda_align": float(LAMBDA_ALIGN),
        "task_precision_config": task_precision_config or {},
    }
...
def refine_proteo_slice2(adata, forced_genes=None):
    ...
    adata = select_top_var_features_with_forced_genes(adata, top_k=PROTEO_GENE_TOPK, forced_genes=forced_genes)
    adata.obsm["image_coor"] = adata.obsm["spatial"].copy()
    return adata
...
def annotate_feature_mz_to_hmdb(
    mz_value,
    mass_arr,
    rid_arr,
    ppm_tol=None,
    topk_candidates=None,
    min_candidate_weight=None,
):
    candidates = []
    mz = float(mz_value)
    ppm_tol = float(PPM_TOL if ppm_tol is None else ppm_tol)
    topk_candidates = int(TOPK_CANDIDATES if topk_candidates is None else topk_candidates)
    min_candidate_weight = float(MIN_CANDIDATE_WEIGHT if min_candidate_weight is None else min_candidate_weight)
    if mass_arr.size == 0:
        return candidates
...
def build_full_metabolite_kb(metabolite_mz_names, gene_id_list, gene_name_list=None, enabled_sources=None, cache_tag="default", kb_precision_params=None):
    enabled = set([str(x).strip().lower() for x in (enabled_sources or ["hmdb", "reactome", "chebi"]) if str(x).strip()])
    precision_params = dict(kb_precision_params or {})
    hmdb_ppm_tol = float(precision_params.get("hmdb_ppm_tol", PPM_TOL))
    hmdb_topk_candidates = int(precision_params.get("hmdb_topk_candidates", TOPK_CANDIDATES))
    hmdb_min_candidate_weight = float(precision_params.get("hmdb_min_candidate_weight", MIN_CANDIDATE_WEIGHT))
    ...
    cache_key = stable_hash(
        ...
        sorted(list(enabled)),
        ION_MODE, hmdb_ppm_tol, hmdb_topk_candidates, hmdb_min_candidate_weight, USE_CHEBI_RELATION,
    ...
    for mz_name in metabolite_mz_names:
        try:
            ...
            mz = None
        candidates = annotate_feature_mz_to_hmdb(
            mz,
            mass_arr,
            rid_arr,
            ppm_tol=hmdb_ppm_tol,
            topk_candidates=hmdb_topk_candidates,
            min_candidate_weight=hmdb_min_candidate_weight,
        ) if (mz is not None and use_hmdb) else []
    ...
    stats = {
        ...
        "hmdb_ppm_tol": float(hmdb_ppm_tol),
        "hmdb_topk_candidates": int(hmdb_topk_candidates),
        "hmdb_min_candidate_weight": float(hmdb_min_candidate_weight),
        "precision_profile": str(precision_params.get("precision_profile", "")),
    }
    out = {
...
def derive_forced_feature_lists_from_kb(met_adata, rna_adata, enabled_sources=None, task_id="warmup"):
    kb_pack = build_full_metabolite_kb(
        ...
        cache_tag=task_id,
        kb_precision_params=get_met_task_precision_config(task_id),
    )
...
# ============================================================================
def _met_only_task_filter():
    raw = os.getenv("MET_ONLY_TASKS", "").strip()
    if not raw:
        return set()
    alias = {
        "5": "task5_he_gene_to_metabolism",
        "task5": "task5_he_gene_to_metabolism",
        "6": "task6_he_metabolism_to_gene",
        "task6": "task6_he_metabolism_to_gene",
        "7": "task7_he_to_metabolism",
        "task7": "task7_he_to_metabolism",
        "8": "task8_he_to_gene_in_metabolomics",
        "task8": "task8_he_to_gene_in_metabolomics",
    }
    out = set()
...
def _should_run_met_task(task_id):
    filt = _met_only_task_filter()
    return (not filt) or str(task_id).strip().lower() in filt
...
def run_metabolomics_tasks_for_seed(repeat_idx, seed, data_pack, gene_lat_pack, orchestration_dir=None):
    ...
    base_trainer_kwargs = dict(
        ...
        device=device,
        lambda_target=LAMBDA_TARGET,
        lambda_pcc=met_clean_loss_value("lambda_pcc", LAMBDA_PCC),
        lambda_latent=met_clean_loss_value("lambda_latent", LAMBDA_LATENT),
        lambda_recon=met_clean_loss_value("lambda_recon", LAMBDA_RECON),
        lambda_ortho=met_clean_loss_value("lambda_ortho", LAMBDA_ORTHO),
        lambda_dgi=met_clean_loss_value("lambda_dgi", LAMBDA_DGI),
        lambda_cycle=LAMBDA_CYCLE,
        lambda_partial_cycle=LAMBDA_PARTIAL_CYCLE,
        lambda_bridge_align=met_clean_loss_value("lambda_bridge_align", LAMBDA_BRIDGE_ALIGN),
        lambda_align=LAMBDA_ALIGN,
        target_soft_alpha=met_clean_loss_value("target_soft_alpha", TARGET_SOFT_ALPHA),
        ...
        disable_dynamic_retriever=DISABLE_DYNAMIC_RETRIEVER,
        disable_kb_fusion=DISABLE_KB_FUSION,
        disable_soft_gate=bool(met_clean_loss_value("disable_soft_gate", DISABLE_SOFT_GATE)),
        disable_residual_refine=DISABLE_RESIDUAL_REFINE,
    )
    ...
    def _resolve_task_kb(task_id, source_modalities, target_modality):
        ...
        kb_pack = build_full_metabolite_kb(
            ...
            cache_tag=task_id,
            kb_precision_params=precision_cfg,
        )
        ...
        if precision_cfg:
            kb_weight_info["lambda_kb_eff"] = float(precision_cfg.get("lambda_kb", kb_weight_info.get("lambda_kb_eff", LAMBDA_KB)))
            kb_weight_info["lambda_graph_eff"] = float(precision_cfg.get("lambda_graph", kb_weight_info.get("lambda_graph_eff", LAMBDA_GRAPH)))
            kb_weight_info["precision_profile"] = str(precision_cfg.get("precision_profile", ""))
        if MET_CLEAN_LOSS:
            kb_weight_info["lambda_graph_eff"] = 0.0
            kb_weight_info["clean_loss"] = True
    ...
    if _should_run_met_task("task5_he_gene_to_metabolism"):
        ...
        apply_met_task_precision_trainer_overrides(trainer5_kwargs, "task5_he_gene_to_metabolism")
        task5_cache = build_met_task_cache_path(
            ...
            full_use_obs=True,
            partial_use_obs=True,
            task_precision_config=kb5.get("precision_config"),
        )
        if (not FORCE_TASK_RETRAIN) and os.path.exists(task5_cache):
            ...
            history5 = trainer5.train()
            pred5 = trainer5.infer_partial()
            gt5 = to_dense(B1_met.X).astype(np.float32)
            pred5_full = pred5["pred_full"]
            m5 = compute_three_metrics(gt5, pred5_full, graph_eval_B1_met)
            run_detailed_task_analysis(
                task_name="task5_he_gene_to_metabolism",
                repeat_idx=repeat_idx,
                seed=seed,
                history=history5,
                gt=gt5,
                pred=pred5_full,
                graph_eval=graph_eval_B1_met,
                adata_for_names_and_coords=B1_met,
                target_task="metabolism",
            )
            row5 = {
    ...
    if _should_run_met_task("task6_he_metabolism_to_gene"):
        ...
        apply_met_task_precision_trainer_overrides(trainer6_kwargs, "task6_he_metabolism_to_gene")
        task6_cache = build_met_task_cache_path(
            ...
            full_use_obs=True,
            partial_use_obs=True,
            task_precision_config=kb6.get("precision_config"),
        )
        if (not FORCE_TASK_RETRAIN) and os.path.exists(task6_cache):
            ...
            history6 = trainer6.train()
            pred6 = trainer6.infer_partial()
            gt6 = to_dense(C1_rna.X).astype(np.float32)
            pred6_full = pred6["pred_full"]
            m6 = compute_three_metrics(gt6, pred6_full, graph_eval_C1_rna)
            run_detailed_task_analysis(
                task_name="task6_he_metabolism_to_gene",
                repeat_idx=repeat_idx,
                seed=seed,
                history=history6,
                gt=gt6,
                pred=pred6_full,
                graph_eval=graph_eval_C1_rna,
                adata_for_names_and_coords=C1_rna,
                target_task="gene",
            )
            row6 = {
                "experiment_id": "E2_unified8", "repeat_idx": int(repeat_idx), "seed": int(seed),
    ...
    if _should_run_met_task("task7_he_to_metabolism"):
        # Task7: HE -> Metabolism
        ...
        apply_met_task_precision_trainer_overrides(trainer7_kwargs, "task7_he_to_metabolism")
        task7_cache = build_met_task_cache_path(
            ...
            full_use_obs=False,
            partial_use_obs=False,
            task_precision_config=kb7.get("precision_config"),
        )
        if (not FORCE_TASK_RETRAIN) and os.path.exists(task7_cache):
            ...
            history7 = trainer7.train()
            pred7 = trainer7.infer_partial()
            gt7 = to_dense(B1_met.X).astype(np.float32)
            pred7_full = pred7["pred_full"]
            m7 = compute_three_metrics(gt7, pred7_full, graph_eval_B1_met)
            run_detailed_task_analysis(
                task_name="task7_he_to_metabolism",
                repeat_idx=repeat_idx,
                seed=seed,
                history=history7,
                gt=gt7,
    ...
    if _should_run_met_task("task8_he_to_gene_in_metabolomics"):
        ...
        apply_met_task_precision_trainer_overrides(trainer8_kwargs, "task8_he_to_gene_in_metabolomics")
        task8_cache = build_met_task_cache_path(
            ...
            full_use_obs=False,
            partial_use_obs=False,
            task_precision_config=kb8.get("precision_config"),
        )
        if (not FORCE_TASK_RETRAIN) and os.path.exists(task8_cache):
            ...
            history8 = trainer8.train()
            pred8 = trainer8.infer_partial()
            gt8 = to_dense(C1_rna.X).astype(np.float32)
            pred8_full = pred8["pred_full"]
            m8 = compute_three_metrics(gt8, pred8_full, graph_eval_C1_rna)
            run_detailed_task_analysis(
                task_name="task8_he_to_gene_in_metabolomics",
                repeat_idx=repeat_idx,
                seed=seed,
                history=history8,
                gt=gt8,
        ...
        json.dump(
            {
                ...
                "e15_aligned_config": {
                    ...
                    "lambda_partial_cycle": LAMBDA_PARTIAL_CYCLE,
                    "lambda_kb": LAMBDA_KB,
                    "lambda_graph": met_clean_loss_value("lambda_graph", LAMBDA_GRAPH),
                    "met_clean_loss": MET_CLEAN_LOSS,
                    "lambda_ortho": LAMBDA_ORTHO,
                    "lambda_dgi": LAMBDA_DGI,
                    "lambda_full": LAMBDA_FULL,
                    "lambda_partial": LAMBDA_PARTIAL,
