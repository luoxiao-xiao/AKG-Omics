...
def structural_similarity_on_graph_data(x, y, adj, K1=0.01, K2=0.03, alpha=1, beta=1, gamma=1, sigma=1.5,
    ...
    if use_sample_covariance:
        cov_norm = num_neighbor_list / (num_neighbor_list - 1 + 1e-6)  # 璁＄畻鏂瑰樊鐨刵orm
    else:
...
def create_ImageEncoder(model_name='resnet50', pretrained=True, frozen=True,
    local_weight_path="weights/pytorch_model.bin",
    device=None):
    """Create the H&E image encoder used by AKG-Omics."""
    ...
    if device.type == "cuda" and device.index is not None and device.index >= torch.cuda.device_count():
        raise ValueError(f"Invalid CUDA device index {device.index}; available devices: {torch.cuda.device_count()}")
    ...
    if model_name == "resnet50":
        ...
        if pretrained:
            model.load_state_dict(torch.load(local_weight_path, map_location="cpu"), strict=True)
...
def Generate_pseudo_spot(adata, key=['x_centroid', 'y_centroid'], platform='visium', all_in=False):
    ...
    if platform == 'visium':  # 瀹氫箟 x 鍜?y 鐨勮寖鍥?        x_interval = 100
    ...
    spot = np.vstack([spot1, spot2])  # 鍒拌繖姝ユ槸鐢熸垚浜嗙煩褰㈢偣闃?
    tree = KDTree(spot)
    distances, indices = tree.query(spatial)
    if all_in:
        in_spot = np.array([True] * adata.n_obs)
    else:
        in_spot = distances < 55 / 2.0  # 杩欓噷娌℃湁鍖呭惈缁嗚優鐨剆pot鑷姩灏变細琚垹闄ゆ帀
    print(in_spot.sum(), ' cells are included in its nearest spot!')
    indices[~in_spot] = -1
    adata.obs['spot'] = indices  # 瀛樺偍瀵瑰簲鐨剆pot鐨刬d

    count = pd.DataFrame(adata[in_spot].X)  # 鑱氬悎寰楀埌姣忎釜spot鐨勫熀鍥犺〃杈?    count.columns = adata.var_names
